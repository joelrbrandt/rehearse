...
OUTPUT_DMG="processing-rehearse"
WORK_DMG="working.dmg"
WORK_DIR="/Volumes/Processing" # where the working.dmg will mount to

gzip -cd template.dmg.gz > "$WORK_DMG"
#mkdir -p "$WORK_DIR" # don't need this cuz we'll let it mount to /Volumes
hdiutil attach "$WORK_DMG" -noautoopen # -mountpoint "$WORK_DIR" # this was causing bugs
for i in "$SOURCE_FILES"; do
	echo Copying $SOURCE_DIR/$i
        rm -rf "$WORK_DIR/$i"
	ditto -rsrc "$SOURCE_DIR/$i" "$WORK_DIR/$i"
done
WC_DEV=`hdiutil info | grep "$WORK_DIR" | awk '{print $1}'`
echo WC_DEV $WC_DEV
